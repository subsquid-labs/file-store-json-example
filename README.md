...
The squid uses [`@subsquid/file-store`](https://docs.sqd.dev/en/sdk) and [`@subsquid/file-store-json`](https://docs.sqd.dev/en/sdk) packages to store the dataset. It outputs its data in the [JSONL](https://jsonlines.org) format instead of the plain JSON array used by default.
